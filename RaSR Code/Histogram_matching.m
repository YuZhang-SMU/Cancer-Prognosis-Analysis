clear
path1='D:\KIRC_Data\'; % File location of the cropped pathes 
files1=dir(path1);
Imatch=imread('Reference image.tiff'); % Download from https://mitos-atypia-14.grand-challenge.org
for n=3:numel(files1)
    path2=[path1,files1(n).name];
    path3=['D:\KIRC_Data\KIRC_his\',files1(n).name,'\']; % File location of pathes after histogram matching 
    mkdir(path3);
    files2=dir(path2);
